...
The configuration of database migration is in the [storage.properties](https://orgonscan.org/api/files/full05.conf#L36) field in the java-orgon  node configuration file. The following is an example of migrating only the `block` and `trans` databases to illustrate how to migrate some databases to other storage disks:
